...
const ConcertList = ({
  ...
  useEffect(() => {
    const fetchConcerts = async () => {
      ...
      const {
        data,
        error
      } = await supabase.from("events").select("id, title, organizer, location, date, price").gte("date", new Date().toISOString().split('T')[0]).order("date", {
        ascending: true
      });
      if (error) {
  ...
  if (loading) {
    return <div className="flex flex-col gap-4 px-4 pb-24">
        <div className="text-center py-8 text-primary">Chargement...</div>
      </div>;
  }
  return <div className="flex flex-col gap-4 pb-24 px-[24px]">
      {displayedConcerts.map(concert => <ConcertCard key={concert.id} concert={concert} onNavigate={() => navigate(`/concert/${concert.id}`)} />)}

      {displayedConcerts.length === 0 && <EmptyState />}

      {hasMore && displayedConcerts.length > 0 && <div ref={loaderRef} className="py-4 text-center">
